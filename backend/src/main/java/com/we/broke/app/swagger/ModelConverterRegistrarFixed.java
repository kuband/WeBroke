...
package com.we.broke.app.swagger;
 ...
 * @author bnasslahsen
 */
public class ModelConverterRegistrarFixed {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ModelConverterRegistrarFixed.class);
     ...
     */
    public ModelConverterRegistrarFixed(List<ModelConverter> modelConverters) {
        modelConvertersInstance.getConverters().stream().filter(modelConverter -> modelConverter instanceof ModelResolver).findFirst().ifPresent(modelConvertersInstance::removeConverter);
        modelConvertersInstance.addConverter(new ModelResolverFixed(new ObjectMapper()));
        for (ModelConverter modelConverter : modelConverters) {
            Optional<ModelConverter> registeredConverterOptional =
                    getRegisteredConverterSameAs(modelConverter);
    ...
    private Optional<ModelConverter> getRegisteredConverterSameAs(ModelConverter modelConverter) {
        try {
            Field convertersField = FieldUtils.getDeclaredField(ModelConverters.class,
                    "converters", true);
            List<ModelConverter> modelConverters =
                    (List<ModelConverter>) convertersField.get(modelConvertersInstance);
     ...
     * @return the boolean
     */
    private boolean isSameConverter(ModelConverter modelConverter1,
